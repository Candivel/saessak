...
class Regex {
  ...
  static bool isEmail(String email) => GetUtils.hasMatch(email,
  ...
  static bool isPassword(String value) => GetUtils.hasMatch(value,
      r'^(?=.*[A-Za-z])(?=.*\d)(?=.*[~!@#$%^&*()+|=_])[A-Za-z\d~!@#$%^&*()+|=]{6,50}$');

  static bool hasBadSpecialCharacterPw(String value) {
    return [
      '-',
      '_',
      '/',
  ...
  /* 이름 : 전체 길이 2~20자 */
  static bool isName(String value) => GetUtils.hasMatch(
      value, r'^[가-힣]{2,10}|[a-zA-Z]{2,10}\s[a-zA:-Z]{2,20}$');
   ...
   * 11. String getRoughFormattingNumber(int num) : 숫자를 만명/억명 단위로 표시함.
   */
  ...
  /* 숫자를 만명/억명 단위로 표시함. */
  static String getRoughFormattingNumber(int num) {
    if (num ~/ 100000000 > 0) {
      if (num % 100000000 == 0) {
        return '${(num ~/ 100000000).toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}억';
      } else {
        return '${((num ~/ 10000000) / 10).toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}억';
      }
    }

    return '${(num ~/ 10000).toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}만';
  }
  ...
  /* 소수점 길이 고정 + 퍼센트 단위 추가 */
  static roundOffNum(double num) {
    return (double.parse(num.toStringAsFixed(2)) * 100).round();
  }
